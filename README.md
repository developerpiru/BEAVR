...
3. In the third column, you can specify additional characteristics for each sample. For example, you can specify different genotype groups like ```wildtype``` or ```knockout```. The heading name for this column must be ```replicate```.
